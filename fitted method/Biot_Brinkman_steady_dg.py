...
def solve_biotBrinkman_steady_dg(h, order_eta, order_u,order_p, fe, fm,fp, exact_eta, exact_u, exact_p, mu,lam,beta_eta,beta_u,gamma_p,alpha,K):

    # 1. Construct the mesh
    # mesh = Mesh(unit_square.GenerateMesh(maxh=h))
    square = SplineGeometry()
    square.AddRectangle((0, 0), (1, 1),bc=1)
    ngmesh = square.GenerateMesh(maxh=h, quad_dominated=False)
    mesh = Mesh(ngmesh)
...
# penalty parameters
# alpha = 10 , beta_eta = 50, beta_u = 10
beta_eta = 200
beta_u = 200
gamma_p = 0
