...
def solve_biotbrinkman_dg(dt, h0, order_eta, order_u,order_p, fe, fm,fp,mu,lam,beta_eta,beta_u,gamma_p,alpha,K):
    ...
    mstar.AsVector().data = Mh.mat.AsVector() + dt*Ah.mat.AsVector()
...
def TimeStepping(invmstar,lh,Mh,initial_condeta = None, initial_condu = None, initial_condp = None, t0 = 0, tend = 1,
                      nsamples = 10):
...
for k in range(2, 6):
    ...
    # dt = h0**3
    endT = dt
    fes,invmstar,lh,Mh,mesh = solve_biotbrinkman_dg(dt, h0, order_eta, order_u,order_p, fe, fm,fp, mu,lam,beta_eta,beta_u,gamma_p,alpha,K)
    t.Set(0)
    gfu = GridFunction(fes)
    gfu = TimeStepping(invmstar,lh, Mh,initial_condeta = exact_eta, initial_condu=exact_u, initial_condp=exact_p, tend=endT, nsamples=20)
    t.Set(endT)
