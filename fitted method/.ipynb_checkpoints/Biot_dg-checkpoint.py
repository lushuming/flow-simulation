...
def solve_biot_dg(dt, F, g, exact_u, exact_p, beta_u=10, beta_p=10, gamma_p=10, order=1, mh=0.1):
    ...
    P = L2(mesh, order=order-1, dirichlet=".*", dgjumps=True) # space for pressure
    fes = U*P
    (u,p), (v,q) = fes.TnT()
    ...
    return fes,invmstar,f,m



def TimeStepping(invmstar, f, m, initial_condu = None, initial_condp = None, t0 = 0, tend = 1,
                      nsamples = 10):
...
for k in range(2, 6):
    mh = 1/2**k
    fes,invmstar,f,m = solve_biot_dg(dt, F, g, exact_u, exact_p, beta_u, beta_p, gamma_p, order, mh)
    gfu = GridFunction(fes)
