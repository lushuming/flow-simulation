...
def solve_biot_eg(dt, F, g, exact_u, exact_p, beta_u=10, beta_p=10, gamma_p=10, order=1, mh=0.1):
    # 1. Construct the mesh
    mesh = Mesh(unit_square.GenerateMesh(maxh=mh))

    # 2. Define the DG space 
    Ucg = VectorH1(mesh, order=order)  # 连续线性空间
    Udg = VectorL2(mesh, order=0,dgjumps=True)  # 不连续常数空间（块状加密）
    Ueg = Ucg*Udg
    
    Pcg = H1(mesh, order=order)  # 连续线性空间
    Pdg = L2(mesh, order=0,dgjumps=True)  # 不连续常数空间（块状加密）
    Peg = Pcg*Pdg
    
    fes = Ueg*Peg
    (ucg,udg,pcg,pdg), (vcg,vdg,qcg,qdg) = fes.TnT()
    u = ucg + udg
    p = pcg + pdg
    v = vcg + vdg
    q = qcg + qdg
    uOther = ucg.Other() + udg.Other()
    pOther = pcg.Other() + pdg.Other()
    vOther = vcg.Other() + vdg.Other()
    qOther = qcg.Other() + qdg.Other()
    ...
    strain_u = Sym(Grad(u))
    strain_v = Sym(Grad(v))
    mean_stress_u = 0.5*(Stress(Sym(Grad(u)))+Stress(Sym(Grad(uOther))))*n
    mean_stress_v = 0.5*(Stress(Sym(Grad(v)))+Stress(Sym(Grad(vOther))))*n
    mean_dpdn = 0.5*K*(grad(p)+grad(pOther))*n
    mean_dqdn = 0.5*K*(grad(q)+grad(qOther))*n
    mean_p = 0.5*(p + pOther)
    mean_q = 0.5*(q + qOther)
    h = specialcf.mesh_size  
    div_u = div(ucg) + div(udg)
    div_v = div(vcg) + div(vdg)
   
    # 4. Define bilinear form
    ah = BilinearForm(fes)
    # Au
    ah += 2*mu*InnerProduct(strain_u,strain_v)*dx + lam*div_u*div_v*dx \
    ...
    # -B
    ah += -alpha*(div_v*p*dx - mean_p*jump_v*n*dx(skeleton=True) - p*v*n*ds(skeleton=True))
    # Ap
    ...
    # B^T
    mh += alpha*(div_u*q*dx - mean_q*jump_u*n*dx(skeleton=True) - q*u*n*ds(skeleton=True))
    mh.Assemble()
...
def TimeStepping(invmstar, f, mh, initial_condu = None, initial_condp = None, t0 = 0, tend = 1,
    ...
    if initial_condu and initial_condp :
        ...
        gfu.components[1].Set(initial_condu-gfu.components[0])
        gfu.components[2].Set(initial_condp)
        gfu.components[3].Set(initial_condp-gfu.components[2])
    cnt = 0; # 时间步计数
    time = t0 # 当前时间
...
for k in range(2, 5):
    ...
    error_u = sqrt(Integrate((gfu.components[0]+gfu.components[1] - exact_u)**2, mesh))
    error_p = sqrt(Integrate((gfu.components[2]+gfu.components[3] - exact_p)**2, mesh))
    ndof = gfu.space.ndof
