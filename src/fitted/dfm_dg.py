...
def solve_dfm_dg(mh,order,sigma,alpha_gamma,beta_gamma,f,u_exact,gD):
...
def print_convergence_table(results):
    print(f"{'h':>8} | {'DoFs':>8} | {'L2 Error (bulk)':>12} | {'Order':>6} | {'L2 Error (fracture)':>12} | {'Order':>6}")
    print("-" * 75)
...
for k in range(2, 6):
    mh = 1/2**k
    ndof, err_bulk, err_fracture = solve_dfm_dg(mh,order,sigma,alpha_gamma,beta_gamma,f,u_exact,gD)
    # L2误差计算
