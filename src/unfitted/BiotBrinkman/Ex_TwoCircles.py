...
def solve_biotbrinkman_steady_unfitted(h0, quad_mesh, order_eta, order_u,order_p, fe, fm,fp, exact_eta, exact_u, \
    ...
    square.AddRectangle((-1, -0.5), (1, 0.5), bc='outer')
    ...
    dsc = dCut(lsetp1, IF, definedonelements=hasif, deformation=deformation)
    ...
    Ah += - (InnerProduct(Stress(Sym(Grad(eta)))*ne,kxi) + InnerProduct(Stress(Sym(Grad(kxi)))*ne,eta))*dsbar
    Ah += - (InnerProduct(Stress(Sym(Grad(eta)))*n,kxi) + InnerProduct(Stress(Sym(Grad(kxi)))*n,eta))*dsc
    Ah += beta_eta/h*(2*mu*InnerProduct(jump_eta,jump_kxi)+lam*InnerProduct(jump_eta,ne)*InnerProduct(jump_kxi,ne)) * dk
    Ah += beta_eta/h*(2*mu*InnerProduct(eta,kxi) + lam*InnerProduct(eta,ne)*InnerProduct(kxi,ne))*dsbar
    Ah += beta_eta/h*(2*mu*InnerProduct(eta,kxi) + lam*InnerProduct(eta,n)*InnerProduct(kxi,n))*dsc
    ...
    # Be
    Ah += -alpha*(div(kxi)*p*domega - mean_p*jump_kxi*ne*dk - p*kxi*ne*dsbar - p*kxi*n*dsc)
    ...
    Ah += nu*InnerProduct(Grad(u),Grad(v))*domega \
            ...
            - nu*(InnerProduct(Grad(u)*n,v) + InnerProduct(Grad(v)*n,u) - beta_u/h*InnerProduct(u,v))*dsc\
    ...
    Ah += gamma_u / (h**2) * (u - u.Other()) * (v - v.Other()) * dw
     ...
     # Bm 
    Ah += -div(v)*p*domega + mean_p*jump_v*ne*dk + p*v*ne*dsbar + p*v*n*dsc
    
    # -Bm
    Ah += div(u)*q*domega - mean_q*jump_u*ne*dk - q*u*ne*dsbar - q*u*n*dsc
    ...
    # -Be
    Ah += alpha*(div(eta)*q*domega - mean_q*jump_eta*ne*dk - q*eta*ne*dsbar - q*eta*n*dsc)
    ...
    # r.h.s
    lh = LinearForm(fes) 
    lh += fe*kxi*domega - InnerProduct(etaD,Stress(Sym(Grad(kxi)))*ne)*dsbar - InnerProduct(etaD,Stress(Sym(Grad(kxi)))*n)*dsc
    lh += beta_eta/h*(2*mu*InnerProduct(etaD,kxi) + lam*InnerProduct(etaD,ne)*InnerProduct(kxi,ne))*dsbar
    lh += beta_eta/h*(2*mu*InnerProduct(etaD,kxi) + lam*InnerProduct(etaD,n)*InnerProduct(kxi,n))*dsc
    lh += fm*v*domega 
    lh += - (nu*InnerProduct(uD,Grad(v)*ne) - nu*beta_u/h*uD*v)*dsbar
    lh += - (nu*InnerProduct(uD,Grad(v)*n) - nu*beta_u/h*uD*v)*dsc
    lh += fp*q*domega 
    lh += - (alpha*q*etaD*ne + q*uD*ne)*dsbar
    lh += - (alpha*q*etaD*n + q*uD*n)*dsc
    lh.Assemble()
...
levelset = IfPos(circle1-circle2,circle1,circle2)
