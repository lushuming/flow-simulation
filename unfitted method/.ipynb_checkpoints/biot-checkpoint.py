...
def solve_biot_dg(dt, F, g, exact_u, exact_p, levelset, quad_mesh=quad_mesh, mu,lam,lambda_u,lambda_p,gamma_s,gamma_p,gamma_m,alpha,M,k, order=1, h=0.1):
    ...
    lh += f*q*dx - alpha*q*uD*n*ds - K*grad(q)*n*pD*ds + lambda_p/h*pD*q*ds
...
def TimeStepping(invmstar, initial_condu = None, initial_condp = None, t0 = 0, tend = 1,
    ...
    while time <  tend + 1e-7:
        t.Set(time)
        lh.Assemble() # 这个不可以提前在lh的定义之后就assemble
...
for k in range(2, 6):
    ...
    fes,invmstar,lh,mh,mesh = solve_biot_dg(dt, F, g, exact_u, exact_p, levelset, quad_mesh=quad_mesh,\
                                            mu,lam,lambda_u,lambda_p,gamma_s,gamma_p,gamma_m,alpha,M,k, order, h0)
    gfu = GridFunction(fes)
