...
def solve_biot_steady_unfitted(h, quad_mesh, orderu,orderp, levelset, b, f, exact_u, exact_p, mu,lam,tau_fpl,lambda_u,lambda_p,gamma_s,gamma_p,alpha,K):
...
def print_convergence_table(results):
    ...
    print("-" * 70)
    for i, (h,dofs,conds,error_u,error_p) in enumerate(results):
        if i == 0:
            ...
            print(f"{h:8.4f} | {dofs:8d} | {conds:12.4e} |{error_u:12.4e}| {rate_u:6.2f} | {error_p:12.4e} | {rate_p:6.2f}")
...
for k in range(2, 6):
    ...
    # tau_fpl = 0.1*h0**2
    tau_fpl = 0
    error_u,error_p, ndof, conds = solve_biot_steady_unfitted(h0, quad_mesh, orderu,orderp, levelset, b, f, exact_u, exact_p, \
                                                                        mu,lam,tau_fpl,lambda_u,lambda_p,gamma_s,gamma_p,alpha,K)
